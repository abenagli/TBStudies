...
int main(int argc, char** argv)
{
  ...
  std::map<std::string,TProfile2D*> p2_eff_vs_hodo1XY;
  std::map<std::string,TProfile2D*> p2_eff_vs_hodo2XY;
  std::map<std::string,TProfile2D*> p2_eff_vs_extrXY;
  std::map<std::string,TProfile2D*> p2_eff_vs_avgXY;
  ...
  for(auto ch : channels)
  {
    ...
    p2_eff_vs_hodo1XY[ch] = new TProfile2D(Form("p2_eff_vs_hodo1XY_%s",ch.c_str()),"",128,-32.,32.,128,-32.,32.);
    p2_eff_vs_hodo2XY[ch] = new TProfile2D(Form("p2_eff_vs_hodo2XY_%s",ch.c_str()),"",128,-32.,32.,128,-32.,32.);
    p2_eff_vs_extrXY[ch] = new TProfile2D(Form("p2_eff_vs_extrXY_%s",ch.c_str()),"",128,-32.,32.,128,-32.,32.);
    p2_eff_vs_avgXY[ch] = new TProfile2D(Form("p2_eff_vs_avgXY_%s",ch.c_str()),"",128,-32.,32.,128,-32.,32.);
  ...
  for(int entry = 0; entry < nEntries; ++entry)
  {
    ...
    for(auto ch: channels)
    {
      ...
      if( (tv.nFibresOnX[0] < 2 || tv.nFibresOnX[0] > 3) ) continue;
      if( (tv.nFibresOnX[1] < 2 || tv.nFibresOnX[1] > 3) ) continue;
      if( (tv.nFibresOnY[0] < 2 || tv.nFibresOnY[0] > 3) ) continue;
      if( (tv.nFibresOnY[1] < 2 || tv.nFibresOnY[1] > 3) ) continue;
      
      float z = opts.GetOpt<float>(Form("%s.zPos",ch.c_str()));
      float xExtr = tv.hodoX[0] + mX * (z-hodo1X_z);
      float yExtr = tv.hodoY[0] + mY * (z-hodo1Y_z);
      float xAvg = 0.5*(tv.hodoX[0] + tv.hodoX[1]);
      float yAvg = 0.5*(tv.hodoY[0] + tv.hodoY[1]);
      ...
      {
        p2_eff_vs_hodo1XY[ch] -> Fill( tv.hodoX[0],tv.hodoY[0],1. );
        p2_eff_vs_hodo2XY[ch] -> Fill( tv.hodoX[1]-3.50,tv.hodoY[1]-0.12,1. );
        p2_eff_vs_extrXY[ch] -> Fill( xExtr,yExtr,1. );
        p2_eff_vs_avgXY[ch] -> Fill( xAvg,yAvg,1. );
        
        if( tv.hodoX[0] > xtalXMin && tv.hodoX[0] < xtalXMax && tv.hodoY[0] > xtalYMin && tv.hodoY[0] < xtalYMax )
        {
          n_tot_hodo1[ch] += 1;
          n_pass_hodo1[ch] += 1;
        }
      ...
      {
        p2_eff_vs_hodo1XY[ch] -> Fill( tv.hodoX[0],tv.hodoY[0],0. );
        p2_eff_vs_hodo2XY[ch] -> Fill( tv.hodoX[1]-3.50,tv.hodoY[1]-0.12,0. );
        p2_eff_vs_extrXY[ch] -> Fill( xExtr,yExtr,0. );
        p2_eff_vs_avgXY[ch] -> Fill( xAvg,yAvg,0. );
  ...
  for(auto ch: channels)
  {
    ...
    c = new TCanvas(Form("c_eff_vs_hodo1XY_%s",label.c_str()),Form("c_eff_vs_hodo1XY_%s",label.c_str()));
    gPad -> SetLogz();
    
    prof2 = p2_eff_vs_hodo1XY[ch];
    prof2 -> SetTitle(Form(";hodo1 X;hodo1 Y"));
    ...
    c -> Print(Form("%s/c_eff_vs_hodo1XY_%s.png",plotDir.c_str(),label.c_str()));
    c -> Print(Form("%s/c_eff_vs_hodo1XY_%s.pdf",plotDir.c_str(),label.c_str()));
    
    
    
    c = new TCanvas(Form("c_eff_vs_hodo2XY_%s",label.c_str()),Form("c_eff_vs_hodo2XY_%s",label.c_str()));
    gPad -> SetLogz();
    
    prof2 = p2_eff_vs_hodo2XY[ch];
    prof2 -> SetTitle(Form(";hodo2 X;hodo2 Y"));
    prof2 -> SetMinimum(0.001);
    prof2 -> SetMaximum(1.1);
    prof2 -> Draw("COLZ");
    ...
    c -> Print(Form("%s/c_eff_vs_hodo2XY_%s.png",plotDir.c_str(),label.c_str()));
    c -> Print(Form("%s/c_eff_vs_hodo2XY_%s.pdf",plotDir.c_str(),label.c_str()));
    
    
    
    c = new TCanvas(Form("c_eff_vs_extrXY_%s",label.c_str()),Form("c_eff_vs_extrXY_%s",label.c_str()));
    gPad -> SetLogz();
    
    prof2 = p2_eff_vs_extrXY[ch];
    prof2 -> SetTitle(Form(";extr X;extr Y"));
    prof2 -> SetMinimum(0.001);
    prof2 -> SetMaximum(1.1);
    prof2 -> Draw("COLZ");
    ...
    c -> Print(Form("%s/c_eff_vs_extrXY_%s.png",plotDir.c_str(),label.c_str()));
    c -> Print(Form("%s/c_eff_vs_extrXY_%s.pdf",plotDir.c_str(),label.c_str()));


    
    c = new TCanvas(Form("c_eff_vs_avgXY_%s",label.c_str()),Form("c_eff_vs_avgXY_%s",label.c_str()));
    gPad -> SetLogz();
    
    prof2 = p2_eff_vs_avgXY[ch];
    prof2 -> SetTitle(Form(";avg X;avg Y"));
    prof2 -> SetMinimum(0.001);
    prof2 -> SetMaximum(1.1);
    prof2 -> Draw("COLZ");
    ...
    c -> Print(Form("%s/c_eff_vs_avgXY_%s.png",plotDir.c_str(),label.c_str()));
    c -> Print(Form("%s/c_eff_vs_avgXY_%s.pdf",plotDir.c_str(),label.c_str()));    
  }
